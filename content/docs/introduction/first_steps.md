...
http://localhost:9090/graph and use the dropdown next to the "Execute" button to see a list of metrics this server is collecting. In the list you'll see a number of metrics prefixed with `node_`, that have been collected by the Node Exporter by our `node` job. For example, `node_exporter_build_info` shows the version information of the exporter.

You can see the node's CPU usage via the `node_cpu_seconds_total` metric. This is a typical counter metric, which can be transformed into a usage graph by applying some functions.
